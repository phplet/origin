 ...
 * @param school_area
 */
function region_province(p_id, school_country, school_province, school_area)
{
	p_id = p_id || 0;
	
    $.getJSON(root_url+'/admin/common/regions/',{'p_id':p_id},function(data) {
        var len = data.length;
        var _depth_1_html = '';
        for(var i=0;i<len;i++){
        ...
        region_city(province_id, school_province, school_area);
	});
}
 ...
 * @param school_area
 */
function region_city(p_id, school_province, school_area)
{
	p_id = p_id || 0;
    ...
    $.getJSON(root_url+'/admin/common/regions/',{'p_id':p_id},function(data) {
        ...
        region_area(city_id, school_area);
	});
}
 ...
 * @param school_area
 */
function region_area(p_id, school_area)
{
	p_id = p_id || 0;
...
function show_school(area_id){
	...
	var grade_id = $("#select_grade_id").val();
	$.getJSON(root_url+'/admin/common/school_list/',{'area_id':area_id,'keyword':keyword,'grade_id':grade_id},function(data) {
        var len = data.length;
        var _school_html = '';
        for(var i=0;i<len;i++){
