...
$(document).ready(function () {
		...
		var change_group_question = function (p_id) {
			...
			if (!$p_question.find('div.c_undo').length) {
				...
				if (!$p_question.find('div.c_done').length) {
					 ...
					 * 完全没做
					 */
					$('#' + $p_question.attr('question_index')).removeClass('done').removeClass('nodone').addClass('undone');
				} else {
					/**
					 * 只答部分
					 */
	...
	var submit_success_redirect = function () {
		$.Zebra_Dialog('恭喜你，交卷成功，系统将在 <strong id="dialog_success_count_down">' + _count_down + '</strong> 秒后跳转到成功提示页.', {
		    'type':     'confirmation',
		    'title':    '系统温馨提示',
		    'buttons':  [],
		    'onClose':  function(caption) {
