...
function refresh_time () {
	...
	if (total_time <= 1 || start_left_time <= 1) {
		...
		if($('p#id_statusbar_clock').html().indexOf("已经结束") > 0){
			
			$('center#tiper_box').hide();
			window.location.href = end_url;
		} 
		else if (start_left_time <= 1) {
...
function redirect_login()
{
	...
	$.post(exam_url, {'hidden_place':$('#hidden_place').val()}, function (data) {
	//	$.cookie("zeming_exam_test", $('#hidden_place').val(), {expires: data, path: "/",domain:domain});
...
$(document).ready(function(){
	...
	$('.place_id').change(function () {
		...
		//ajax获取当前时间
		var ajax_url = '/public.php';
		//$.get(ajax_url, {'act':"current_time"}, function (data) {
			
			//_exam_count_down_config.currentTime = val;
