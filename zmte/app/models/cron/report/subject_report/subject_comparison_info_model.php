 ...
 * @final 2015-07-21
 */
class Subject_comparison_info_model extends CI_Model 
{
    private static $_db;
	 ...
	 */
	public function module_knowledge($rule_id = 0, $exam_id = 0, $uid = 0)
	{
		...
		$paper_id = $this->_get_student_exam_paper($uid, $exam_id);
		if (!$paper_id)
		{
			return array();
		}
	 ...
	 * @param number $uid 考生ID
	 */
	public function module_group_type($rule_id = 0, $exam_id = 0, $uid = 0)
	{
	    $rule_id = intval($rule_id);
	    $exam_id = intval($exam_id);
	    $uid = intval($uid);
	 ...
	 * @param number $uid 考生ID
	 */
	public function module_method_tactic($rule_id = 0, $exam_id = 0, $uid = 0)
	{
		$rule_id = intval($rule_id);
		$exam_id = intval($exam_id);
		$uid = intval($uid);
	 ...
	 * @param number $uid 考生ID
	 */
	public function module_difficulty($rule_id = 0, $exam_id = 0, $uid = 0)
	{
		$rule_id = intval($rule_id);
		$exam_id = intval($exam_id);
		$uid = intval($uid);
	...
	/**
	 * 知识点和题组类型（信息提取方式）比例 模块
	 * @param number $rule_id 评估规则ID
	 * @param number $exam_id 考试学科
	 * @param number $uid 考生ID
	 */
	public function module_knowledge_group_type($rule_id = 0, $exam_id = 0, $uid = 0)
	{
	    $rule_id = intval($rule_id);
	    $exam_id = intval($exam_id);
	    $uid     = intval($uid);
	    ...
	    //外部对比信息
	    $comparison_data = $this->get_comparison_item_extraction_ratio($rule_id, $exam_id);
	    $exams = $comparison_data['exams'];
	    $extraction_ratios = $comparison_data['extraction_ratio'];
	    ...
	    if (empty(self::$_data['extraction_ratio'][$exam_id][$paper_id]))
	    {
	        $sql = "SELECT full_score FROM rd_exam_test_paper
	               WHERE exam_id={$exam_id} AND paper_id={$paper_id}";
	        $full_score = self::$_db->fetchOne($sql);
	    
	        //获取试卷信息
            $sql = "SELECT q.ques_id,q.knowledge,q.group_type
                    FROM rd_exam_question eq 
                    LEFT JOIN rd_question q ON eq.ques_id = q.ques_id 
                        OR eq.ques_id = q.parent_id
                    WHERE paper_id = $paper_id AND q.children_num = 0";
	        $query = self::$_db->query($sql);
	        $ques_k_gt = array('0'=>array(),'1'=>array());
            while ($ques_val = $query->fetch(PDO::FETCH_ASSOC))
            {
            	if ($ques_val['knowledge'] != '')
            	{
            	    $ques_k_gt[0][] = $ques_val['ques_id'];
            	}
            	else
            	{
            	    $ques_k_gt[1][] = $ques_val['ques_id'];
            	}
            }
            
	        $extraction_ratio_name = array('知识点','信息提取方式');

	        foreach($ques_k_gt as $key => $item)
	        {
	            $ques_ids = implode(',', $item);
	            
	            if (empty($ques_ids)) continue;

	            $sql = "SELECT SUM(test_score) FROM rd_exam_test_result
        	            WHERE exam_id={$exam_id} AND (ques_id IN({$ques_ids}) OR sub_ques_id IN({$ques_ids}))
        	            AND uid = $uid
        	            ";
	            $score = self::$_db->fetchOne($sql);
	            $percent = $score / $full_score * 100;
	        
	            //收集内部
	            $_name = $extraction_ratio_name[$key];
	            $extraction_ratios['internal'][] = $_name;
	            $exams[$exam_name][$_name] = $percent;
	        }
	        
	        //过滤重复
	        $extraction_ratios['internal'] = array_values(array_unique($extraction_ratios['internal']));
	
	        self::$_data['extraction_ratio'][$exam_id][$paper_id] = array(
	            'extraction_ratio' => $extraction_ratios, 
	            'exams' => $exams
	        );
	    }
	    
	    return self::$_data['extraction_ratio'][$exam_id][$paper_id];
	}
	 ...
	 * 学生考试的paper_id
	 */
	private function _get_student_exam_paper($uid, $exam_id)
	{
