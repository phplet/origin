...
*/
    	foreach ($result as $row)
    	{
    		...
    		if ($row['type'] > 0)
    		{
    		    $row['score'] = isset($exam['qtype_score_arr'][$row['type']-1]) ? $exam['qtype_score_arr'][$row['type']-1] : 0;
    ...
    public function get_paper_question_detail_p($paper_id, $userdata = array())
    {
       ...
       // if(!$result)
      //  {
            $sql = "SELECT q.ques_id,q.type,q.title,q.score_factor,rc.difficulty,q.answer
            FROM {pre}exam_question eq
            LEFT JOIN {pre}question q ON eq.ques_id=q.ques_id
            LEFT JOIN {pre}relate_class rc ON rc.ques_id=q.ques_id AND rc.grade_id='$exam[grade_id]' AND rc.class_id='$exam[class_id]'
            WHERE eq.paper_id=$paper_id  ORDER BY rc.difficulty DESC,q.ques_id ASC";
    ...
    //end
        ...
        $new_result = array();
        $new_result_r = array();
        $sql = "SELECT count(etr.etr_id) as 'count',etr.ques_id,etr.answer from {pre}exam_test_result etr,{pre}exam_test_paper etp
        where  etp.etp_id=etr.etp_id and etr.paper_id={$paper_id} and etr.uid={$userdata['uid']} and etr.exam_id={$exam_id} group by etr.ques_id " ;
        $result1 = $this->db->query($sql)->result_array();
        foreach ( $result1 as $row1)
        {
            $new_result[$row1['ques_id']]=$row1['count'];
            $new_result_r[$row1['ques_id']]=$row1['answer'];
        }

        //查询用户所有答案未做的
        $new_result_undo = array();

        $sql = "SELECT count(etr.etr_id) as 'count',etr.ques_id from {pre}exam_test_result etr,{pre}exam_test_paper etp
        where  etp.etp_id=etr.etp_id and etr.paper_id={$paper_id} and etr.uid={$userdata['uid']} and etr.exam_id={$exam_id} and etr.answer ='' group by etr.ques_id ";
        ...
        foreach ($result as $row)
        {
            ...
            if($row['ques_id']==9365)
            {
                echo $new_result_undo[$row['ques_id']];
                echo '/';
                echo $new_result[$row['ques_id']];
                die;
            }
            */
            ...
            elseif( $new_result[$row['ques_id']] ==1)
            {
                ...
                else
                {
                    if($row['type']==3)
                    {
                        ...
                        if($row['ques_id']==2219)
                        {
                            var_dump($new_result_r[$row['ques_id']]);
                            echo count(array_filter(explode("\n",$new_result_r[$row['ques_id']])));
                            echo '/';
                            echo count(array_filter(explode("\n",$row['answer'])));
                            die;
                        }
                        */
                        if(count(array_filter(explode("\n",$new_result_r[$row['ques_id']]))) ==count(array_filter(explode("\n",$row['answer']))))
                        {
                            $row['do_or_undo']='done';
                        }
                        else
