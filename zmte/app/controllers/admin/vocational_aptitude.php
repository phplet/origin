 ...
 */
class Vocational_aptitude extends A_Controller
{
    //职业能力倾向
    protected static $pr_subjectid = 16;
    ...
    public function save()
    {
        ...
        try 
        {
            if ($param['pr_id'])
            {
                ...
                admin_log('edit', 'vocational_aptitude', $param['pr_id']);
            }
            else 
            {
                $result = ProfessionRelatedModel::addProfessionRelated($param);
                admin_log('add', 'vocational_aptitude', $result);
            }
            
            message(($param['pr_id'] ? '编辑' : '新增') . ($result ? '成功' : '失败'), 
                '/admin/vocational_aptitude/index');
        }
        catch (Exception $e)
        {
            message('职业能力倾向' . $e->getMessage());
        }
    }
